const CACHE='spruce-shell-v18';
const SHELL=['./','./index.html','./styles.css','./app.js','./demo.js','./sync-ui.js','./sync-model.js','./model.js','./schema.js','./manifest.json'];
...
self.addEventListener('fetch',event=>{const url=new URL(event.request.url);if(event.request.method!=='GET'||url.origin!==self.location.origin||url.pathname==='/health'||url.pathname.startsWith('/api/'))return;event.respondWith(caches.match(event.request).then(cached=>cached||fetch(event.request)));});
